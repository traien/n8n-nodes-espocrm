...
export class MetadataService {
	 ...
	 */
	static async getFieldDefs(execFunctions: IFunctions, entityType: string): Promise<IDataObject> {
		const endpoint = `/Metadata`;
		const responseData = await espoApiRequest.call(execFunctions, 'GET', endpoint);
	 ...
	 */
	static async getEntityList(execFunctions: IFunctions): Promise<string[]> {
		const endpoint = '/Metadata';
		const responseData = await espoApiRequest.call(execFunctions, 'GET', endpoint);
	 ...
	 */
	static async getEntityLinks(execFunctions: IFunctions, entityType: string): Promise<IDataObject> {
		const endpoint = `/Metadata`;
		const responseData = await espoApiRequest.call(execFunctions, 'GET', endpoint);
	 ...
	 */
	static generateFieldOptions(fieldDefs: IDataObject, fieldName: string): INodePropertyOptions[] {
